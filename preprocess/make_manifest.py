    ...
    for fname in json_list:
        ...
        with open(fname, "r") as f:
            ...
            new_sentence = sentence_filter(
                raw_sentence=transcription, mode=args.preprocess_mode
            )
            ...
            file_info.append(
                "{}\t{}".format(os.path.relpath(file_path, args.root), frames)
            )
            transcriptions.append(new_sentence)
            texts.append(" ".join(list(new_sentence.replace(" ", "|"))) + " |")

    print(f"[{data_name}] Length ignore numbers : {limit_count}")
    print(f"[{data_name}] Digit and Alphabet ignore numbers : {remove_count}")
...
def save_files(args, dataset, file_info, transcriptions, texts):
    ...
    ) as ltr_out, open(os.path.join(args.dest, dataset + ".wrd"), "w") as wrd_out:
...
def save_dict(args, transcriptions):
    ...
    with open(os.path.join(args.dest, "dict.ltr.txt"), "w") as write_f:
        for idx, (grpm, freq) in enumerate(zip(vocab_list, vocab_freq)):
            print(f"{grpm} {freq}", file=write_f)
...
def clean_up(dir_path):
    ...
    for fname in file_list:
        if os.path.exists(fname):
            os.remove(fname)
            print(f"[{fname}] already exists -> removed")
...
def main(args):
    ...
    for folder in ["1.Training", "2.Validation"]:
        ...
        ), f"root 경로를 확인해주세요. [{args.root}]"
        for dir in ["라벨링데이터", "원천데이터"]:
            if dir not in os.listdir(os.path.join(args.root, folder)):
                assert os.path.isdir(folder), "'{}' 폴더를 찾을 수 없습니다. [{}]".format(
                    dir, os.path.join(args.root, folder, dir)
                )
    ...
    data_name = "train"
    train_file_info, train_transcriptions, train_texts = load_script(
        args, data_name, train_json_list
    )
    save_files(args, data_name, train_file_info, train_transcriptions, train_texts)
    save_dict(args, train_transcriptions)

    data_name = "valid"
    valid_file_info, valid_transcriptions, valid_texts = load_script(
        args, data_name, valid_json_list
    )
    save_files(args, data_name, valid_file_info, valid_transcriptions, valid_texts)

    data_name = "test"
    test_file_info, test_transcriptions, test_texts = load_script(
        args, data_name, test_json_list
    )
