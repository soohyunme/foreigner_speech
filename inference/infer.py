...
output units",
    ...
    parser.add_argument(
        "--w2l-decoder",
        choices=["viterbi", "kenlm", "parlance", "fairseqlm"],
...
def main(args, task=None, model_state=None):
    ...
    def build_generator(args):
        ...
        if w2l_decoder == "viterbi":
            ...
            print(
                "only flashlight decoders with (viterbi, kenlm, parlance, fairseqlm) options are supported at the moment"
            )
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            # logger.info(f'| | hypos = {len(hypos)}')
            # for _h in hypos:
            #     logger.info(f'| | | token = {_h[0]["tokens"]}')
            # import pdb; pdb.set_trace()
            # exit()
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                ...
                errs, length, _cer = process_predictions(
                    args,
                    hypos[i],
                    None,
                    tgt_dict,
